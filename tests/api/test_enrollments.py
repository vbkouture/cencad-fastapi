...
@pytest.fixture
async def enrollment_repo(setup_db: AsyncGenerator[None, None]) -> AsyncGenerator[EnrollmentRepository, None]:
...
async def student_token() -> str:
    ...
    token = create_access_token(
        data={"sub": student_id, "role": UserRole.STUDENT}
    )
    return token
...
class TestCreateEnrollment:
    ...
    async def test_create_enrollment_success(
        self, client: AsyncClient, student_token: str, cleanup_enrollments: AsyncGenerator[None, None]
    ) -> None:
...
class TestGetMyEnrollments:
    ...
    async def test_get_my_enrollments(
        self, client: AsyncClient, student_token: str, enrollment_repo: EnrollmentRepository, cleanup_enrollments: AsyncGenerator[None, None]
    ) -> None:
...
class TestUpdateEnrollment:
    ...
    async def test_update_enrollment_status(
        self, client: AsyncClient, enrollment_repo: EnrollmentRepository, cleanup_enrollments: AsyncGenerator[None, None]
    ) -> None:
