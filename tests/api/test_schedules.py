...
@pytest.fixture
async def schedule_repo(setup_db: AsyncGenerator[None, None]) -> AsyncGenerator[ScheduleRepository, None]:
...
async def tutor_token() -> str:
    ...
    token = create_access_token(
        data={"sub": "test_tutor_id", "role": UserRole.TUTOR}
    )
    return token
...
def _create_valid_schedule_payload() -> dict[str, any]:  # type: ignore[name-defined]
...
class TestGetSchedules:
    ...
    async def test_get_schedule_by_id(
        self, client: AsyncClient, schedule_repo: ScheduleRepository, cleanup_schedules: AsyncGenerator[None, None]
    ) -> None:
...
class TestUpdateSchedule:
    ...
    async def test_update_schedule_success(
        self, client: AsyncClient, schedule_repo: ScheduleRepository, admin_token: str, cleanup_schedules: AsyncGenerator[None, None]
    ) -> None:
        ...
        headers = {"Authorization": f"Bearer {admin_token}"}
        response = await client.put(f"/api/v1/schedules/{str(schedule['_id'])}", json=payload, headers=headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
